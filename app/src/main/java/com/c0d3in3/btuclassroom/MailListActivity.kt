...
class MailListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mailRecyclerView.adapter = MailAdapter(mailsList, this)
            ...
            override fun onPostExecute(result: Void?) {
                ...
                main.forEach{ele ->
                    ...
                    if(ele.className().toString() == "info"){
                        mailsList.add(Mails(count, arg[1].text(), arg[2].text(), arg[3].text(), arg[1].select("a").attr("href").toString(), true))
                    }
                    else{
                        mailsList.add(Mails(count, arg[1].text(), arg[2].text(), arg[3].text(), arg[1].select("a").attr("href").toString(), false))
                    }
                    count += 1
                }
                if(count != 0){
